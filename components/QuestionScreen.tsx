...
const QuestionScreen: React.FC<Props> = ({ onYes }) => {
  ...
  const handleYes = () => {
    if (CONFIG.emailMode === 'emailjs') {
      sendEmailNotification().catch(err => console.error("Email failed:", err));
    }
    onYes();
  };
  ...
  const microcopy = TEASING_MICROCOPY[Math.min(noAttempts, TEASING_MICROCOPY.length - 1)];

  return (
    <div ref={containerRef} className="flex flex-col items-center justify-center space-y-16 animate-in fade-in zoom-in duration-1000">
